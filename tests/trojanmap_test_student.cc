...
//   std::cout << "My min path length: "  << result.first << "miles" << std::endl; // Print the result path lengths
//   std::vector<std::string> gt{"1873056015", "213332060", "1931345270", "6905329551", "1873056015"}; // Expected order
//   std::cout << "GT min path length: "  << m.CalculatePathLength(gt) << "miles" << std::endl; // Print the gt path lengths
//   bool flag = false;
...
//Test TSP function for 2opt
// TEST(TrojanMapTest, TSP2opt) {
//   TrojanMap m;
//   m.CreateGraphFromCSVFile();
//   std::vector<std::string> input{"123120189", "4011837229", "4011837224", "2514542032", "2514541020", "1931345270", "4015477529", "214470792", "63068532", "6807909279"}; // Input location ids 
...
TEST(TrojanMapTest, CycleDetection) {
 ...
 // Test case 3
  std::vector<double> square3 = {-118.290112, -118.137, 34.011, 34.123};
...
TEST(TrojanMapTest, TopologicalSort2) {
  ...
  m.CreateGraphFromCSVFile();
  std::vector<std::string> location_names = {"Cardinal Gardens", "Coffee Bean1","CVS"};
  std::vector<std::vector<std::string>> dependencies = {{"Coffee Bean1","Cardinal Gardens"}, {"CVS","Cardinal Gardens"}, {"Coffee Bean1","CVS"}};
  auto result = m.DeliveringTrojan(location_names, dependencies);
  std::vector<std::string> gt ={"Coffee Bean1","CVS","Cardinal Gardens"};
  EXPECT_EQ(result, gt);
}
...
TEST(TrojanMapTest, TopologicalSort3) {
  TrojanMap m;
  m.CreateGraphFromCSVFile();
  std::vector<std::string> location_names = {"Target", "Ralphs","CVS"};
  std::vector<std::vector<std::string>> dependencies = {{"Target","Ralphs"}, {"CVS","Ralphs"}, {"Target","CVS"}};
  auto result = m.DeliveringTrojan(location_names, dependencies);
  std::vector<std::string> gt ={"Target","CVS","Ralphs"};
  EXPECT_EQ(result, gt);
}